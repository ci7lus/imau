...
export const DiffFetchButton: React.FC<{
  ...
  const handleClick = useCallback(async () => {
    ...
    try {
      ...
      const queryAniListIdByAnnictId = (annictId: number) =>
        arm.find((entry) => entry.annict_id === annictId)?.anilist_id ?? null
      ...
      const diffs = works
        .map((work) => {
          ...
          const status = serviceStatuses.find((status) => status.id === workId)
          if (
            !status ||
  ...
  return (
    <Button color={!isFetching ? undefined : "red"} onClick={handleClick}>
